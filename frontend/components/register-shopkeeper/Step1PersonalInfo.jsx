...
import { User, Phone, Mail, Lock, Check, AlertCircle, Loader2, Save, ShieldCheck, Sparkles, X, CheckCircle2, Pencil } from 'lucide-react';
...
export const Step1PersonalInfo = ({
  ...
  const handleSendPhoneOtp = async () => {
    ...
    setOtpError(null);
    try {
      const res = await API.post('/auth/send-otp', { phoneNumber: '+91' + phone });
      setOtpId(res.data.data.otpId);
      setOtpSent(true);
      setOtpDigits(['', '', '', '', '', '']);
      const mockOtp = res.data.data.mockOtp;
      showToast('Phone OTP Sent!', `Verification code sent to +91 ${phone}${mockOtp ? ` (Demo Code: ${mockOtp})` : ''}`);
    } catch (err) {
  ...
  const handleSendEmailOtp = async () => {
    ...
    try {
      // Send OTP (Backend automatically checks for existing user email first and returns 409 if exists)
      const res = await API.post('/auth/send-email-otp', { email: regEmail });
      setEmailOtpId(res.data.data.otpId);
      setEmailOtpSent(true);
      setEmailOtpDigits(['', '', '', '', '', '']);
      const mockOtp = res.data.data.mockOtp;
      showToast('Email OTP Sent!', `Verification code sent to ${regEmail}${mockOtp ? ` (Demo Code: ${mockOtp})` : ''}`);
    } catch (err) {
      ...
      if (err.response?.status === 409 || errorMsg.toLowerCase().includes('already exists')) {
        ...
        setEmailOtpError(errorMsg);
        showToast('Account Check Failed', errorMsg, 'error');
      }
  ...
  const handleSaveDraft = () => {
    ...
    localStorage.setItem('merchant_step1_draft', JSON.stringify(draft));
    ...
    <div className="space-y-5 relative">
      ...
      <div className="space-y-2">
        ...
        {otpSent && !phoneVerified && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="p-4 bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3 mt-2">
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 font-heading">Enter 6-Digit Phone OTP Code</label>
      ...
      <div className="space-y-2">
        ...
        {emailOtpSent && !emailVerified && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="p-4 bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3 mt-2">
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 font-heading">Enter 6-Digit Email OTP Code</label>
